...
namespace Odin.ClientApi.Factory;
...
public sealed class SharedSecretSystemTextJsonContentSerializer : IHttpContentSerializer
{
    ...
    public HttpContent ToHttpContent<T>(T item)
    {
        ...
        return JsonContent.Create(payload, payload.GetType(), MediaTypeHeaderValue.Parse("application/json"),
    ...
    public async Task<T?> FromHttpContentAsync<T>(HttpContent content, CancellationToken cancellationToken = default)
    {
        ...
        {
            var c = await (new ByteArrayContent(decryptedBytes)).ReadFromJsonAsync<T>(jsonSerializerOptions, cancellationToken)
                .ConfigureAwait(false);
            // var rawJson = decryptedBytes.ToStringFromUtf8Bytes();
            // var c = OdinSystemSerializer.Deserialize<T>(rawJson);
            return c;
        }
