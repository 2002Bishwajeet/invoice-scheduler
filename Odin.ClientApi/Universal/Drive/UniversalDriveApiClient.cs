...
public class LocalClientDriveData
{
...
{
    public async Task<List<LocalClientDriveData>> GetDrives(Guid driveType,
        CancellationToken cancellationToken = default)
    {
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity);
        var apiResponse = await svc.GetDrivesByType(driveType, 1, int.MaxValue, cancellationToken);

        if (apiResponse.IsSuccessStatusCode && apiResponse.Content != null)
        {
            return apiResponse.Content.Results.Select(d => new LocalClientDriveData
            {
                Name = d.Name,
        ...
        return new List<LocalClientDriveData>();
    }
        ...
        FileSystemType fileSystemType = FileSystemType.Standard)
    {
        ...
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity, fileSystemType);
        ...
        ApiResponse<UploadResult> response = await svc.UploadStream(parts.ToArray());
        ...
        FileSystemType fileSystemType = FileSystemType.Standard)
    {
        ...
        var t = transitOptions ?? new TransitOptions()
        {
        ...
        FileSystemType fileSystemType = FileSystemType.Standard)
    {
        ...
        var sharedSecret = factory.SharedSecret;
        {
            ...
            var fileDescriptorCipher = Utils.MiscUtils.JsonEncryptAes(descriptor, uploadInstructionSet.TransferIv, sharedSecret);
            ...
            ApiResponse<FileUpdateResult> response = await svc.UpdateFile(parts.ToArray());
        ...
        FileSystemType fileSystemType = FileSystemType.Standard)
    {
        ...
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity, fileSystemType);
        {
            ApiResponse<UpdateLocalMetadataResult> response = await svc.UpdateLocalMetadataTags(request);
        ...
        FileSystemType fileSystemType = FileSystemType.Standard)
    {
        ...
        var sharedSecret = factory.SharedSecret;
        {
            ApiResponse<UpdateLocalMetadataResult> response = await svc.UpdateLocalMetadataContent(request);
    ...
    public async Task<(
        ApiResponse<FileUpdateResult> response,
    ...
    public async Task<ApiResponse<SharedSecretEncryptedFileHeader>> GetFileHeaderByClientUniqueId(Guid uniqueId, TargetDrive targetDrive,
        FileSystemType fileSystemType = FileSystemType.Standard)
    {
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity, fileSystemType);
        var apiResponse = await svc.GetFileHeaderByClientUniqueId(uniqueId, targetDrive.Alias, targetDrive.Type);
        return apiResponse;
    }

    public async Task<ApiResponse<SharedSecretEncryptedFileHeader>> GetPayloadByClientUniqueId(Guid uniqueId, TargetDrive targetDrive,
        string payloadKey, FileSystemType fileSystemType = FileSystemType.Standard)
    {
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity, fileSystemType);
        var apiResponse = await svc.GetPayloadByClientUniqueId(uniqueId, targetDrive.Alias, targetDrive.Type, payloadKey);
        return apiResponse;
    }

    public async Task<ApiResponse<SharedSecretEncryptedFileHeader>> GetThumbnailByClientUniqueId(Guid uniqueId, TargetDrive targetDrive,
        string payloadKey, FileSystemType fileSystemType = FileSystemType.Standard)
    {
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity, fileSystemType);
        var apiResponse = await svc.GetThumbnailByClientUniqueId(uniqueId, targetDrive.Alias, targetDrive.Type, payloadKey);
        return apiResponse;
    }
    ...
    {
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity);
        var response = await svc.GetOutboxItem(drive.Alias, drive.Type, fileId, recipient);
        return response;
    }
    ...
    {
        var svc = factory.CreateRefitHttpClient<IDriveHttpClientApi>(identity);
        var response = await svc.SendReadReceipt(new SendReadReceiptRequest
        {
            Files = files
        });
